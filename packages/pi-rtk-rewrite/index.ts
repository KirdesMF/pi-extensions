...
export default function rtkRewriteExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand(KEY, {
		description:
			"Manage RTK bash rewriting: /rtk-rewrite [status|on|off|refresh|test <cmd>|help]",
		handler: async (args, ctx) => {
			...
			switch (subcommand) {
				...
				case "status": {
					...
					ctx.ui.notify(
						buildStatusMessage(config, sessionEnabled, rtkAvailable),
						"info",
					);
					return;
				}
				...
				case "test": {
					...
					if (!rtkAvailable) {
						ctx.ui.notify(
							"RTK not available. Install RTK and ensure `rtk rewrite` works in PATH.",
							"warning",
						);
						return;
					}
					...
					if (rewritten) {
						...
						ctx.ui.notify(`No RTK rewrite for:\n\n${rawCommand}`, "info");
					}
					return;
				}
				case "help": {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (rtkAvailable === false && ctx.hasUI) {
			ctx.ui.notify(
				"RTK rewrite extension loaded, but `rtk rewrite` not available from PATH.",
				"warning",
			);
		}
	});
	...
	async function getRewrite(
		...
		try {
			...
			// Ignore RTK failures. Original bash command should still run.
		}
...
function buildStatusMessage(
	...
	const availability =
		rtkAvailable === null ? "checking" : rtkAvailable ? "available" : "missing";
	return [
