...
const CUSTOM_TOOL_EXTENSIONS: Record<string, string> = {
	web_search: path.join(TOOLS_DIR, "web-search.ts"),
	web_fetch: path.join(TOOLS_DIR, "web-fetch.ts"),
...
// Simple visibleWidth replacement: measure string width excluding ANSI escapes
...
// Per-event hard cap on stored arg previews.
				...
				if (evt.type === "message_end" && evt.message) {
					...
					if (msg.role === "assistant") {
						...
						if (u) {
							...
							// NOT a cumulative sum across turns.
							progress.tokens =
								u.totalTokens ||
								(u.input || 0) +
									(u.output || 0) +
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const result = await semaphore.run(() =>
				runSubagent(
					agent,
					params.task,
					(params.cwd as string | undefined) ?? cwd,
					signal,
					(progress, usage) => {
			...
			return {
				...
				details: { results: [result] } as Details,
		...
		renderCall(args, theme, context) {
			...
			if (!context.expanded) {
				...
				const taskPreview = args.task
					...
					: "";
				return new Text(
					`${theme.fg("toolTitle", theme.bold("subagent"))} ${theme.fg("accent", args.agent as string)} ${theme.fg("dim", taskPreview)}`,
					0,
					0,
				);
			}

			// Expanded view: header + full streaming task body.
			let c: Container;
			...
			if (args.task) {
				c.addChild(new Spacer(1));
				c.addChild(new Text(theme.fg("text", args.task as string), 0, 0));
			}
			return c;
		},
